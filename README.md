...
```
You may also pass `.json` and/or `.css` files as model and style to create a Lenode. Once created, subnodes can be accessed as well as their methods as properties. `myNode.buttonEnter.onclick = () => foo();` or `myNode.divInfo.addClass('highlight');`, etc.
